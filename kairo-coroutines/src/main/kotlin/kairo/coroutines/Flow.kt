 ...
 * Returns the single element, or null if the flow is empty.
 * If the flow has more than one element, throws [IllegalArgumentException].
 *
 * This is less surprising than [singleOrNull]
 * because it doesn't return null when there are multiple items in the flow.
 */
public suspend fun <T> Flow<T>.singleNullOrThrow(): T? {
  var single: T? = null
  var found = false
  collect { element ->
    require(!found) { "Flow has more than one element." }
    single = element
    found = true
  }

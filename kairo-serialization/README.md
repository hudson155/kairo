...
- One of these key ways is that Jackson's exceptions are very rich,
  including the exact problem details, the JSON path to the offending field, and even the source location.
...
Being such an old library, Jackson has some baggage.
...
but you must explicitly opt in using `@OptIn(ProtectedString.Access::class)`.

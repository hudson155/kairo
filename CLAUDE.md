...
Feature modules are installed via `kairo-xxx-feature` (e.g., `kairo-rest-feature`, `kairo-sql-feature`).

Use the BOM (`bom` or `bom-full`) for aligned dependency versions.
...
Integration tests use `kairo-integration-testing` with `setup`, `test`, `postcondition` blocks and JUnit extensions (e.g., `@ExtendWith(UserFeatureTest::class)`).

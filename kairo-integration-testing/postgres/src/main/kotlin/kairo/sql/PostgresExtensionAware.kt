...
public interface PostgresExtensionAware {
  public var ExtensionContext.connectionFactory: SqlFeatureConfig.ConnectionFactory?
    get() = getStore(PostgresExtension.namespace).get<SqlFeatureConfig.ConnectionFactory>("connectionFactory")
    set(value) {
      getStore(PostgresExtension.namespace).put("connectionFactory", value)
    }

  public var ExtensionContext.database: Database?
    get() = getStore(PostgresExtension.namespace).get<Database>("database")
    set(value) {
      getStore(PostgresExtension.namespace).put("database", value)
    }

  public var ExtensionContext.databaseName: String?
    get() = getStore(PostgresExtension.namespace).get<String>("databaseName")
    set(value) {
      getStore(PostgresExtension.namespace).put("databaseName", value)
    }

  public val ExtensionContext.postgres: PostgreSQLContainer?
    get() = root.getStore(PostgresExtension.namespace).get<PostgreSQLContainer>("postgres")
